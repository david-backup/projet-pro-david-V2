...
const useModalState = (navigate) => {
  ...
  const loginModalConfig = useCallback(
    (title, message) => ({
      isOpen: true,
      title: title,
      message: message,
  ...
  const successModalConfig = (message, additionalOnClose = () => {}) => ({
    isOpen: true,
    title: "Succès",
    message,
  ...
  const confirmationModalConfig = (
    ...
    icon2: message2 ? (
      <BsExclamationTriangle className="modal-alert-icon" />
    ) : null,
